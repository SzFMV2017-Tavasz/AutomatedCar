 ...
 */
public class InWay {
    private static Rectangle2D.Double RectangleCrafter(IWorldObject obj){
    ...
    public static List<IWorldObject> InWaySortCarFront(AutomatedCar car,List<IWorldObject> objectsin){
        ...
        Rectangle2D.Double carRect= RectangleCrafter(car);
        for(IWorldObject i:objectsin){
            if(IsPointInRect(carRect,new Point2D.Double(i.getCenterX(),i.getCenterY()))){
                returnL.add(i);
            }
        }
        return returnL;
